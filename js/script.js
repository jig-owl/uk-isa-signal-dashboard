document.getElementById("analyze-btn").addEventListener("click", async () => {
    ...
    try {
        ...
        if (data.error) {
            ...
            const c = data.currency || "$";  // fallback to USD if missing
            resultDiv.innerHTML = `
                <strong>Ticker:</strong> ${data.ticker}<br>
                <strong>Signal:</strong> ${data.signal}<br>
                <strong>Reason:</strong> ${data.reason}<br>
                <strong>Price:</strong> ${c}${data.price}<br>
                <strong>Trend:</strong> ${data.trend}<br>
                <strong>RSI:</strong> ${data.rsi}<br>
                <strong>Position Size:</strong> ${c}${data.position_size}<br>
                <strong>Risk per Trade:</strong> ${c}${data.risk_per_trade}<br>
                <strong>Stop Price:</strong> ${c}${data.stop_price}<br>
            `;
        }
    } catch (err) {
